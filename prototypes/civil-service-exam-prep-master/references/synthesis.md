# 公务员考试培训 · 行业 OS 提炼 (Synthesis)

> 研究日期: 2026-05-27 | 来源: 6 轨调研 (01-figures / 02-tools / 03-workflows / 04-canon / 05-sources / 06-glossary) | 总来源: 171 条
...
### 核心工作流（详见 references/research/03-workflows.md）

10 个完整工作流覆盖：

1. **考生备考全流程** (9 步 SOP: 报名→笔试→面试→上岸)
2. **行测备考** (5 模块 ROI 排序 + 120 分钟 135 题考场策略)
3. **申论备考** (材料阅读法→要点提取→题型框架→大作文→时政积累)
4. **面试备考** (结构化 6 大题型 + 对练 + 反模板化)
5. **培训机构课程设计** (考情分析→课件→试讲→迭代)
6. **模考组织** (出题→组卷→考试→评分→数据闭环)
7. **协议班运营** (招生→授课→退费/录取 + 风险管理)
8. **名师 IP 打造** (定位→内容矩阵→粉丝运营→变现)
9. **AI+公考** (AI 批改/AI 面试/智能路径 + 边界)
10. **选岗报名决策** (职位表→条件筛选→竞争比→时机策略)

### 入门 vs 资深的关键差异

| 环节 | 入门做法 | 资深做法 |
|------|---------|---------|
| 行测复习 | 5 模块平均分配 | 资料分析+判断推理吃 60% 时间 |
| 申论阅读 | 三遍读法 | 一遍精读+边标注 |
| 面试准备 | 背 6 大题型模板 | 练思维逻辑，用聊天感表达 |
| 选岗 | 看职位名称选 | 多维评分卡 + 实时竞争比 |
...
### 9.1 考情与政策维度
- **轴**: 国考/省考最新公告、大纲变化、招录政策趋势
- **触发**: 用户提到具体考试年份或政策变化
- **输出**: 从 scs.gov.cn / 各省人社厅官网获取一手公告，对比往年变化

### 9.2 岗位竞争维度
- **轴**: 目标岗位的报录比、历年进面分数线、竞争格局
- **触发**: 用户问「这个岗位值得报吗」或选岗相关问题
- **输出**: 从相对面/粉笔报岗数据查询实时竞争比，对比历史数据

### 9.3 备考策略维度
- **轴**: 笔试(行测+申论)+面试的备考方法论和时间规划
- **触发**: 用户问「怎么备考」或具体科目提分方法
- **输出**: 按 ROI 排序推荐学习路径，区分入门/资深/冲刺不同阶段

### 9.4 工具与课程维度
- **轴**: 培训机构/APP/AI 工具的对比选型
- **触发**: 用户问「用什么工具」或「报哪家课」
- **输出**: 按考生身份×预算×短板维度推荐工具组合，标注优劣势

### 9.5 行业商业维度
- **轴**: 三巨头竞争格局、商业模式（协议班/AI 班/OMO）、行业趋势
- **触发**: 用户从从业者/投资人视角讨论行业
- **输出**: 引用上市公司年报数据+行业报告，区分一手/二手

### 9.6 AI+教育维度
- **轴**: AI 工具在公考培训中的应用现状和边界
- **触发**: 用户问 AI 批改/AI 面试/智能学习路径相关问题
- **输出**: 列出具体 AI 产品、定价、效果数据，明确标注 AI 能做和不能做的边界

### 9.7 体制内认知维度
- **轴**: 公务员职业发展、体制内工作实情、考公动机分析
- **触发**: 用户问「考上之后怎样」或职业规划相关
