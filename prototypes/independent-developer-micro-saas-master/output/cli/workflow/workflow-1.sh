...
explain() {
  cat <<'EOT'
这个 SOP 走查脚本帮你完成 "定价策略设计与迭代" 工作流。

一句话: 从竞品参考到可持续增长的定价结构
...
declare -a SOP_STEPS=(
  "竞品分析: 找 5-10 个竞品，记录其定价页结构、层级数量、价格区间 evidence: [T03-S009]"
  "WTP (Willingness-to-Pay) 测试: 向 20 个目标用户问\"你会为解决 X 每月付多少\"，取中位数而非最高值"
  "设计 3 层定价 (Free/Starter/Pro 或 Starter/Growth/Scale)，月付年付均提供，年付折扣 20-40%"
  "以低于成熟竞品 20-30% 上线，明确说\"早鸟价\"，给自己涨价空间"
  "第一个季度收集真实付款数据: 哪个层级转化最高，哪个功能最常被询问升级"
  "每季度复盘一次，决定是否调价 (通常创业者定价偏低，应上调) evidence: [T03-S009]"
)
