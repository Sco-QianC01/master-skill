...
WORKFLOW="Idea 验证 → 首个付费用户"

usage() {
  cat <<EOF
idea.sh — Walk through "${WORKFLOW}" SOP for ${INDUSTRY}
...
explain() {
  cat <<'EOT'
这个 SOP 走查脚本帮你完成 "Idea 验证 → 首个付费用户" 工作流。

一句话: 从模糊想法到口袋里的第一笔真实付款
...
declare -a SOP_STEPS=(
  "列出自己遇到过的 10 个工作/生活痛点，筛选出\"反复发生且愿意付钱解决\"的 3 个 (跳过后果: 造无人买的产品)"
  "用 Mom Test 框架做 15-20 次用户访谈，只问过去行为，不提产品 (跳过后果: 收到礼貌性假需求)"
  "建一页 Landing Page (Carrd/Framer)，标明价格，加 Stripe Payment Link (跳过后果: 无法判断真实支付意愿)"
  "在目标用户聚集地 (Reddit/Twitter/Slack 社区) 发布，明确说\"我在研究 X 问题\" (跳过后果: 流量为零，验证停滞)"
  "若 48 小时内 100 访客中有 3+ 人点击\"立即购买\"，视为信号成立 evidence: [T03-S001]"
  "收到第一笔付款前不写一行代码 (跳过后果: 浪费 2-3 个月在无人要的功能上)"
)
...
if [[ $JSON -eq 1 ]]; then
  ...
  ms_emit_md_file "idea-walkthrough"
fi
