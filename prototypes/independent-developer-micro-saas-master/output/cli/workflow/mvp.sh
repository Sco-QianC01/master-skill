...
usage() {
  cat <<EOF
mvp.sh — Walk through "${WORKFLOW}" SOP for ${INDUSTRY}
...
declare -a SOP_STEPS=(
  "选技术栈: 2025 主流为 Next.js + Supabase + Stripe 或 Rails + Heroku，用 boilerplate (ShipFast/SaaS Starter) 节省 2 周 evidence: [T03-S013]"
  "只实现\"核心价值循环\"的最小功能集 (≤3 个核心功能)，写 SPEC.md 列出不做什么"
  "本地开发 + Vercel/Railway 部署，首日即可上线，避免自建服务器运维"
  "集成 Stripe Billing + 基础 Auth (Clerk/NextAuth)，确保付款流程可用"
  "写一份简单 README/onboarding email，确保新用户 5 分钟内能体验核心价值 (跳过后果: 注册即流失)"
  "发布到 Product Hunt、HackerNews Show HN、IndieHackers，同步宣布"
)
...
if [[ $JSON -eq 1 ]]; then
  ...
  ms_emit_md_file "mvp-walkthrough"
fi
