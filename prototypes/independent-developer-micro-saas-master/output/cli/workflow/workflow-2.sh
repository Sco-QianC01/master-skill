...
explain() {
  cat <<'EOT'
这个 SOP 走查脚本帮你完成 "客户留存与流失率控制" 工作流。
...
declare -a SOP_STEPS=(
  "Onboarding 设计: 注册后 5 分钟内让用户体验到\"aha moment\"，用 Intercom/Crisp 发送欢迎序列邮件 evidence: [T03-S008]"
  "激活率监控: 用 Mixpanel/PostHog 追踪\"完成核心动作的注册用户%\"，目标 >40%"
  "早期信号识别: 登录频率下降 >50%、连续 7 天未登录 = 高流失风险，立即触发人工联系"
  "挽留实验: 取消前弹出问卷 (Churnkey/ProfitWell Retain)，提供暂停/降级选项而非直接取消"
  "建立\"客户成功\"节奏: 每月向活跃用户发送使用报告 + 功能更新，增加粘性"
  "每季度分析流失用户的共同特征，反馈给产品路线图 evidence: [T03-S008]"
)
