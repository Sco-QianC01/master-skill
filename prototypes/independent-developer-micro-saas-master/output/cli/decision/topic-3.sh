...
explain() {
  cat <<'EOT'
这个决策助手把 产品 主题下的 playbook 规则组合成交互树。

基于 1 条规则:
  • 如果 你不确定从哪个产品形态开始 → 则 : 走 stair-step — 先做一次性销售产品 (WordPress 插件/Shopify 主题/Notion 模
...
declare -a RULE_ACTIONS=(
  ": 走 stair-step — 先做一次性销售产品 (WordPress 插件/Shopify 主题/Notion 模板), 证明你能收钱并处理客服; 再做 micro-SaaS。案例: Rob Walling 从 WordPress 插件 HitTail 起步, 再做 Drip (T04-S001)"
)
