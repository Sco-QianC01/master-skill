# Synthesis: 留学咨询规划 (Study Abroad Consulting)

> locale: global (EN primary ~65% + zh-CN secondary ~30%)
> profile: practitioner
> research_date: 2026-05-24
> source_count: 145 (across 6 tracks)
...
当面对一个新的留学咨询���题时，按���下 7 个维度做功课再回答:

### 9.1 院校招生政策维度
- **看什么**: 目标校的 CDS 数据 + 录取率趋势 + 该校近年 institutional priorities (扩招/缩招/diversity 变化)
- **怎么查**: CDS Initiative 网站 → 对应年份 Section C (录取数据); 学校招生办博客/FAQ
- **判断标准**: 该校近 3 年的 yield 和 enrollment 数据是否暗示某种招生策略变化

### 9.2 学生-学校适配维度
- **看什么**: 五维 fit 矩阵 (学术强度/文化/地理/费用/就业) 对这个具体学生的匹配度
- **怎么查**: Naviance scattergram + 校友就业数据 (LinkedIn) + Niche 学生评价 + net price calculator
- **判断标准**: ≥ 3 个维度高度匹配 = Match; 2 个维度匹配 = 需讨论; 仅 1 个维度匹配 = 不推荐

### 9.3 政策/签证风险维度
- **看什么**: 目标国/目标校的签证通过��� + 学生的具体 profile 风险点 (专业/国籍/资金来源)
- **怎么查**: travel.state.gov 签证统计 + ICE SEVIS 数据 + IRCC processing times + 使领馆最新通知
- **判断标准**: 高风险 flag (敏感 STEM / 资金逻辑链断�� / 多次拒签史) 需在选校阶段就考虑 plan B

### 9.4 竞争格局维度
- **看什么**: 目标校今年的中国/国际申请者竞争态势 + 本校/本地区历史录取数据
- **怎么查**: Naviance/Scoir 同校数据 + 行业社群信息 (HECA/IECA) + 招生办 info session 信号
- **判断标准**: 同校 ≥ 5 人申请同一校 = 高竞争; 需要差异化策略或调整名单

### 9.5 财务可行性维度
- **看什么**: 家庭 4 年总预算 vs 目标校真实总费用 (tuition + room + board + insurance + travel)
- **怎么查**: 每校 net price calculator + CSS Profile 要求 + merit aid 历史数据 + 汇率趋势
- **判断标准**: 总费用 > 家庭预算 120% = 不可行 (除非获得 aid 的概��很高); 需要提前 run 多个 scenario

### 9.6 时间线/deadline 维度
- **看什么**: 当前日期 vs 最近 deadline 的距离 + 所有并行任务的冲突
- **怎么查**: Common App/UCAS/各校官网 deadline + 考试报名截止 + 签证预��� availability
- **判断标准**: deadline < 14 天 = 紧急; < 7 天 = 最高优先级; 多个 deadline 同周 = 需要 triage

### 9.7 伦理/合规维度
- **看什么**: 当前建议是否涉及灰色地带 (代写/夸大/利益冲突/隐瞒信息)
- **怎么查**: NACAC SPGP + IECA 伦理���则 + 目标校 honor code + 本地法规
- **判断标准**: 任何「如果被发现后果不可承受」的操作 = 红线; 任何需要对学生/家长隐瞒的操作 = 红线
