#!/usr/bin/env bash
# agentic.sh — Agentic Protocol CLI for 留学咨询规划.
...
declare -a DIM_QUESTIONS=(
  "目标校的 CDS 数据 + 录取率趋势 + 该校近年 institutional priorities (扩招/缩招/diversity 变化)"
  "五维 fit 矩阵 (学术强度/文化/地理/费用/就业) 对这个具体学生的匹配度"
  "目标国/目标校的签证通过��� + 学生的具体 profile 风险点 (专业/国籍/资金来源)"
  "目标校今年的中国/国际申请者竞争态势 + 本校/本地区历史录取数据"
  "家庭 4 年总预算 vs 目标校真实总费用 (tuition + room + board + insurance + travel)"
  "当前日期 vs 最近 deadline 的距离 + 所有并行任务的冲突"
  "当前建议是否涉及灰色地带 (代写/夸大/利益冲突/隐瞒信息)"
)
